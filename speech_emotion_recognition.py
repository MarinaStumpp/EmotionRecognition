...
class SpeechEmotion(object):
    ...
    def __init__(self, speech_data_available, speech_feature_type, speech_epochs, speech_runs, speech_audio_pickle_path,
                 speech_isConfusion, speech_ravdess_path, **kwargs):
    ...
    def train_model(self, speech_model_type, **kwargs):
        ...
        for i in range(self.runs):
            x_train, x_test, y_train, y_test, encoder = self.prepare_training_data(self.data_available,
                                                                                   self.feature_type)
            ...
            history = model.fit(x_train, y_train, validation_data=(x_test, y_test), batch_size=64, epochs=self.epochs,
                                callbacks=[rlrp], shuffle=True)
    ...
    def extract_features(self, data_path, feature_type, **kwargs):
        features, labels = [], []
        if not os.path.exists('feature_data/' + feature_type + '_features.npy') and not os.path.exists(
                '_feature_data/' + feature_type + '_lables.npy'):
